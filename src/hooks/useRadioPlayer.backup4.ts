...
export function useRadioPlayer() {
  ...
  // Play station with mobile-optimized settings
  const playStation = useCallback((station: RadioStation) => {
    ...
    audio.preload = 'metadata'; // Changed from 'none' for better mobile support
    audio.volume = Math.min(1, Math.max(0, volume / 100));
    
    // Don't set crossOrigin for mobile compatibility
    ...
    // Start playback with better error handling for mobile
