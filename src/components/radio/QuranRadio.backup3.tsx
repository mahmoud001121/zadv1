...
import { Loader2, AlertCircle, Star, Clock, Radio as RadioIcon, BookOpen, Mic, Heart as HeartIcon, Sparkles } from 'lucide-react';
...
export function QuranRadio() {
  ...
  const categorizeStation = (station: RadioStation): string => {
    ...
    // Tafsir category
    if (name.includes('تفسير') || name.includes('tafsir')) {
      return 'tafsir';
    }
    
    // Special categories (not Quran recitation)
  ...
  const addToRecent = (stationId: number) => {
    setRecentPlayed((prev) => {
      ...
      const newRecent = [stationId, ...filtered].slice(0, 10); // Keep last 10
